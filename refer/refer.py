...
class REFER:
    ...
    def export_test_images(self, output_dir):
        """
        将测试集(test split)的图片提取到指定文件夹中
        """
        # 获取测试集的 ref_id 和 image_id
        ref_ids = self.getRefIds(split='test')
        image_ids = self.getImgIds(ref_ids=ref_ids)

        # 创建输出目录
        os.makedirs(output_dir, exist_ok=True)

        print(f"共有 {len(image_ids)} 张测试集图片需要导出...")
        count = 0
        ...
        print(f"✅ 已成功导出 {count} 张测试集图片到: {output_dir}")


if __name__ == '__main__':
    refer = REFER('/remote-home/cs_tcci_liangbocheng/RIS_Datasets/RRSIS-D/', dataset='rrsisd', splitBy='unc')
    ref_ids = refer.getRefIds()

    # 导出测试集图片
    output_dir = '/remote-home/cs_tcci_liangbocheng/mymodel/RMSIN/checkpoints/rrsisd_test_images'
    refer.export_test_images(output_dir)
