...
data_root = "/root/user/cs_tcci_liangbocheng/RIS_Datasets"

def build_rsris_batches(setname):
    im_dir1 = f'{data_root}/RISBench/images/'
    seg_label_dir = f'{data_root}/RISBench/masks/'
    if setname == 'train':
    ...
    n_batch = 0
    train_ids = []
    tf = f'{data_root}/RISBench/'+setfile
    nn = 0
    imgnames = set()
    imname = 'start'
    all_imgs1 = []
    ...
    with open(tf,'r') as rf:
        ...
        for idx,line in enumerate(rlines):
            lsplit = line.split(' ')
            if True:
                im_name1 = im_dir1 + lsplit[0]
                seg = seg_label_dir + lsplit[0]
                del(lsplit[0])
...
class ReferDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_name = os.path.basename(this_img1)  # ✅ 新增：提取文件名
        ...
        # ✅ 返回 img_name
        return img1, target, tensor_embeddings, attention_mask, img_name
